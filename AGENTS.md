...
No env-var auth bootstrap. `init_db()` only creates the schema and runs additive `ALTER TABLE` migrations; duplicate-column / duplicate-index `OperationalError`s are swallowed (idempotency), any other DDL error re-raises. After the DDL pass `_assert_schema_ready()` fails fast if a required column is missing, so a botched migration is loud at startup instead of silent at request time. `init_db()` does **not** auto-create any user. On a fresh install the users table is empty until `POST /api/auth/setup` runs. That endpoint is gated by `db.query(User).count() == 0` and only succeeds when the table is empty — first sign-up becomes admin, and the gate slams shut. `GET /api/auth/setup-required` is the public probe the login page uses to decide whether to render its signup variant.
...
Sessions are invalidated per-user via a `users.session_version` column embedded as `sv` in the JWT payload. `revoke_user_sessions(user)` bumps the column; `current_user()` rejects any cookie whose `sv` doesn't match. Fires on password change, admin password reset, TOTP confirm, TOTP disable, and admin disable. The endpoints that touch the *current* user's credentials (`/change-password`, `/totp/confirm`, `/totp/disable`) re-issue a fresh cookie with the bumped `sv` in the same response so the active tab isn't immediately logged out. Two escape hatches remain:

- **Single user, immediate**: disable from `/admin`. `current_user()` checks `disabled_at` and rejects on the next request (and `session_version` is bumped too as defense-in-depth).
...
- `issue_jwt(user)` → JWT with `sub`, `sv` (session version), `iat`, `exp`, `jti`, set as an HttpOnly+Secure+SameSite=Lax cookie. Cookie name from `SESSION_COOKIE_NAME` env (default `focus_session`).
...
- `current_user()` is the FastAPI dependency every protected route uses; it rejects rows with `disabled_at` set, cookies whose `sv` doesn't match `users.session_version`, and (after auth checks) accounts still pending admin approval.
